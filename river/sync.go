...
import (
	...
	"github.com/JasonYe88/go-mysqlbinlog-to-es/elastic"
...
func (r *River) getFieldParts(k string, v string) (string, string, string) {
	mysqlCol, _, elasticPath, fieldType := parseFieldMapping(k, v)
	return mysqlCol, elasticPath, fieldType
}
...
func (r *River) makeInsertReqData(req *elastic.BulkRequest, rule *Rule, values []interface{}) {
	...
	colIndex := make(map[string]int, len(rule.TableInfo.Columns))
	for i, c := range rule.TableInfo.Columns {
		colIndex[c.Name] = i
	}

	// Columns fully remapped by exact name (no JSON path), skip original column name.
	fullyMapped := make(map[string]bool)
	// Columns with only JSON path mappings: extract fields, do not write raw JSON blob.
	pathOnly := rule.pathOnlyColumns()

	for k, v := range rule.FieldMapping {
		mysqlCol, mysqlPath, elasticPath, fieldType := parseFieldMapping(k, v)
		idx, ok := colIndex[mysqlCol]
		if !ok {
			continue
		}
		c := rule.TableInfo.Columns[idx]
		...
		if mysqlPath == "" {
			fullyMapped[mysqlCol] = true
			fieldValue = r.getFieldValue(&c, fieldType, values[idx])
		} else {
			colValue := r.makeReqColumnData(&c, values[idx])
			fieldValue = applyFieldType(getByPath(colValue, mysqlPath), fieldType)
		}
		setByPath(req.Data, elasticPath, fieldValue)
	}
	...
	for i, c := range rule.TableInfo.Columns {
		...
		if fullyMapped[c.Name] || pathOnly[c.Name] {
			continue
		}
		// Path mapping may already have written this top-level key.
		if _, exists := req.Data[c.Name]; exists {
			continue
		}
...
func (r *River) makeUpdateReqData(req *elastic.BulkRequest, rule *Rule,
	...
	for i, c := range rule.TableInfo.Columns {
		...
		if !reflect.DeepEqual(beforeValues[i], afterValues[i]) {
			changedCols[c.Name] = true
		}
	}

	fullyMappedChanged := make(map[string]bool)
	pathOnly := rule.pathOnlyColumns()

	for k, v := range rule.FieldMapping {
		mysqlCol, mysqlPath, elasticPath, fieldType := parseFieldMapping(k, v)
		if !changedCols[mysqlCol] {
			continue
		}
		idx, ok := colIndex[mysqlCol]
		if !ok {
		...
		var fieldValue interface{}
		if mysqlPath == "" {
			fullyMappedChanged[mysqlCol] = true
			fieldValue = r.getFieldValue(&c, fieldType, afterValues[idx])
		} else {
			colValue := r.makeReqColumnData(&c, afterValues[idx])
			fieldValue = applyFieldType(getByPath(colValue, mysqlPath), fieldType)
		}
		setByPath(req.Data, elasticPath, fieldValue)
	}
	...
	for i, c := range rule.TableInfo.Columns {
		if !changedCols[c.Name] || fullyMappedChanged[c.Name] || pathOnly[c.Name] {
			continue
		}
		if _, exists := req.Data[c.Name]; exists {
			continue
